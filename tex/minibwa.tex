...
BWA-MEM3 is a new fork of BWA-MEM2 with speedup from latency hiding and more fast path to skip full dynamic programming.
...
On real data, this involves Smith-Waterman alignment~\citep{Smith:1981aa} between a $\sim$150 bp read $P$ and a reference substring $R$ of several hundred basepairs.
...
$$
is the number of $k$-mer matches between $P$ and $R[t,t+|P|)$.
$\max_t M_t$ gives us a hint about the best ungapped alignment between $P$ and $R$.
...
Minibwa applies chaining normally and performs base alignment and mate rescue entirely in the converted space.

\section{Results}

We evaluated the mapping accuracy on simulated short and long reads and
...
Minibwa competes with minimap2 and Winnowmap2 for mapping accurate long reads (Fig.~\ref{fig:roc}e and~\ref{fig:roc}f).

All BS-seq aligners in Fig.~\ref{fig:roc}d use the 3-base strategy and can map normal WGS reads in theory.
...
We are not sure why BISCUIT is less accurate than minibwa.
...
The theoretical accuracy as is shown in Fig.~\ref{fig:roc} may not reflect accuracy in downstream analyses.
...
Alignment output is redirected to {\tt /dev/null} except for Bismark which has to generate BAM files.}\label{fig:perf}
\end{figure*}
...
It trades the full compatibility with BWA-MEM for deeper algorithmic improvement
which enables long-read alignment and doubles the performance over BWA-MEM2.
As the developer of BWA-MEM and minimap2, we take minibwa as the full replacement of BWA-MEM.
