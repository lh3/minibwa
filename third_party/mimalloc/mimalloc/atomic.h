...
#elif defined(__GNUC__) && !defined(__clang__) && !defined(__STDC_NO_ATOMICS__) && (__GNUC__ < 4 || (__GNUC__ == 4 && __GNUC_MINOR__ < 9))
// Use GCC __atomic builtins for older GCC (4.7-4.8) that lack stdatomic.h
// Use macros instead of functions since __atomic_* builtins are type-generic
#define  _Atomic(tp)              volatile tp
#define  MI_ATOMIC_VAR_INIT(x)    x
#define  mi_atomic(name)          mi_atomic_##name
#define  mi_memory_order(name)    mi_memory_order_##name

#define mi_memory_order_relaxed   __ATOMIC_RELAXED
#define mi_memory_order_consume   __ATOMIC_CONSUME
#define mi_memory_order_acquire   __ATOMIC_ACQUIRE
#define mi_memory_order_release   __ATOMIC_RELEASE
#define mi_memory_order_acq_rel   __ATOMIC_ACQ_REL
#define mi_memory_order_seq_cst   __ATOMIC_SEQ_CST

// Type-generic atomic macros using GCC builtins
#define mi_atomic_load_explicit(p,mo)                         __atomic_load_n(p,mo)
#define mi_atomic_store_explicit(p,x,mo)                      __atomic_store_n(p,x,mo)
#define mi_atomic_exchange_explicit(p,x,mo)                   __atomic_exchange_n(p,x,mo)
#define mi_atomic_compare_exchange_weak_explicit(p,e,d,ms,mf) __atomic_compare_exchange_n(p,e,d,1,ms,mf)
#define mi_atomic_compare_exchange_strong_explicit(p,e,d,ms,mf) __atomic_compare_exchange_n(p,e,d,0,ms,mf)
#define mi_atomic_fetch_add_explicit(p,x,mo)                  __atomic_fetch_add(p,x,mo)
#define mi_atomic_fetch_sub_explicit(p,x,mo)                  __atomic_fetch_sub(p,x,mo)
#define mi_atomic_fetch_and_explicit(p,x,mo)                  __atomic_fetch_and(p,x,mo)
#define mi_atomic_fetch_or_explicit(p,x,mo)                   __atomic_fetch_or(p,x,mo)
#else
// Use C11 atomics
#include <stdatomic.h>
...
#if defined(__GNUC__) && !defined(__clang__) && !defined(__cplusplus) && (__GNUC__ < 4 || (__GNUC__ == 4 && __GNUC_MINOR__ < 9))
// Old GCC (4.7-4.8): use generic __atomic builtins (they are type-generic)
static inline int64_t mi_atomic_addi64_relaxed(volatile int64_t* p, int64_t add) {
  return mi_atomic_fetch_add_explicit((_Atomic(int64_t)*)p, add, mi_memory_order_relaxed);
}
static inline void mi_atomic_void_addi64_relaxed(volatile int64_t* p, const volatile int64_t* padd) {
  const int64_t add = mi_atomic_load_explicit((_Atomic(int64_t)*)padd, mi_memory_order_relaxed);
  if (add != 0) {
    mi_atomic_fetch_add_explicit((_Atomic(int64_t)*)p, add, mi_memory_order_relaxed);
  }
}
static inline void mi_atomic_maxi64_relaxed(volatile int64_t* p, int64_t x) {
  int64_t current = mi_atomic_load_explicit((_Atomic(int64_t)*)p, mi_memory_order_relaxed);
  while (current < x && !mi_atomic_compare_exchange_strong_explicit((_Atomic(int64_t)*)p, &current, x, mi_memory_order_release, mi_memory_order_relaxed)) { /* nothing */ };
}
// Used by timers
#define mi_atomic_loadi64_acquire(p)            mi_atomic_load_explicit(p,mi_memory_order_acquire)
#define mi_atomic_loadi64_relaxed(p)            mi_atomic_load_explicit(p,mi_memory_order_relaxed)
#define mi_atomic_storei64_release(p,x)         mi_atomic_store_explicit(p,x,mi_memory_order_release)
#define mi_atomic_storei64_relaxed(p,x)         mi_atomic_store_explicit(p,x,mi_memory_order_relaxed)
#define mi_atomic_casi64_strong_acq_rel(p,e,d)  mi_atomic_compare_exchange_strong_explicit(p,e,d,mi_memory_order_acq_rel,mi_memory_order_acquire)
#define mi_atomic_addi64_acq_rel(p,i)           mi_atomic_fetch_add_explicit(p,i,mi_memory_order_acq_rel)
#else
// C++/C11: use polymorphic atomics
static inline int64_t mi_atomic_addi64_relaxed(volatile int64_t* p, int64_t add) {
